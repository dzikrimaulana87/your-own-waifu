...
// Tokenizer untuk memisahkan teks dan menghapus stopwords
function tokenize(text) {
    const words = text.toLowerCase().replace(/[^a-z0-9\u00C0-\u024F]+/gi, ' ').split(/\s+/).filter(Boolean);
    return words.filter(word => !stopwords.includes(word));  // Hapus stopwords
}

// Buat vocab dan vectorize
...
function handleUserInput() {
    ...
    // Ambil semua pattern dari dataset
    let allPatterns = [];
    let tagMap = [];
